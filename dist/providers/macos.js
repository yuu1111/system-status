...
function vendorForName(name) {
    const value = name.toLowerCase();
    if (value.includes("nvidia"))
        return "nvidia";
    if (value.includes("amd") || value.includes("radeon"))
        return "amd";
...
export function parseSystemProfiler(output) {
...
export async function getMacosStatus(runner, timeoutMs = 10_000) {
    const diagnostics = [];
    const [cpuName, physicalCores, diskOutput, profilerOutput] = await Promise.all([
        runner("sysctl", ["-n", "machdep.cpu.brand_string"], timeoutMs).catch(() => undefined),
        runner("sysctl", ["-n", "hw.physicalcpu"], timeoutMs).catch(() => undefined),
        runner("df", ["-kP"], timeoutMs).catch(() => undefined),
        runner("system_profiler", ["SPDisplaysDataType", "-json"], timeoutMs).catch(() => undefined),
    ]);
    if (!diskOutput)
        diagnostics.push({
            provider: "macos-df",
